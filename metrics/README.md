...
- Developer use for pre-checking code changes before PRing
- As part of report generation

## Goals

This section add some details of the goals for the potential use cases.
...
on if we want the test to take a more repeatable measurement (less noise, variance), or if
...
To that end, we set the quality requiments tighter than the PR CI, but allow more runtime:
...
Each sub-folder containers its own `README` detailing its own tests.
...
In order to ensure all of the tests can be run in unison and generate a uniform set
of data that can then be analysed or stored in a uniform manner, we provide some
bash function APIs to ensure such uniformity.

Presently we supply two APIs (JSON and CSV). The CSV API is being deprecated, and the
JSON API is therefore recommended.
...
- These sets could be a hangover from a previous CSV based test - consider using a single JSON file if possible.
...
> Note: document exactly what is accepted here?

#### metrics_json_start_array()

Initialse the JSON array API subsystem, ready to accept JSON fragments via
`metrics_json_add_array_element`.

Thie JSON array API subset allows accumulation of multiple entries into a
