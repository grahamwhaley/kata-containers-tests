...
for (currentdir in resultdirs) {
	...
	for (testname in testnames) {
		...
		for (ffound in files) {
			...
			cdata=data.frame(avail_gb=as.numeric(fdata$Results$mem_free$Result)/(1024*1024))
			cdata=cbind(cdata, cpu_idle=as.numeric(fdata$Results$cpu_idle$Result))
			cdata=cbind(cdata, boot_time=as.numeric(fdata$Results$launch_time$Result))
